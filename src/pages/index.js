...
const IndexPage = () => (
  <Layout>
    <SEO title="Superfish - The jQuery drop-down menu plugin" keywords={[`jQuery`, `Superfish`, `Menu`, `Plugin`]} />
    ...
    <div className="section content">
      ...
      <ul>
        ...
        <li>Supports the <strong>hoverIntent plugin</strong>. Superfish automatically detects the presence of Brian Cherne&#8217;s <a className="external" href="https://github.com/briancherne/jquery-hoverIntent" title="visit the home of hoverIntent">hoverIntent plugin</a> and uses its advanced hover behaviour for the mouseovers (mouseout delays are handled by Superfish regardless of the presence of hoverIntent). Using this is only an option, but a nice one. The examples on this site <em>are</em> using hoverIntent. If for some reason you want to use hoverIntent on your page for other plugins but do not want Superfish to use it you can set the option <PrismCode className="language-javascript">disableHI</PrismCode> to <PrismCode className="language-javascript">true</PrismCode>. <strong>Important note:</strong> As of v1.5.1 you must use the <a href="https://github.com/briancherne/jquery-hoverIntent">updated version (r7) of hoverIntent</a> which supports event delegation.</li>
        ...
        <li>Provides <strong>public methods</strong> <PrismCode className="language-javascript">hide</PrismCode> and <PrismCode className="language-javascript">show</PrismCode> that you can call on list items that have submenu children. Also provides a <PrismCode className="language-javascript">destroy</PrismCode> method for uninitialising Superfish. Here is an <a href="example.html">example page demonstrating the methods in action</a></li>
      ...
      <hr />
			...
			<ol className="steps">
				<li className="step-one">
					Begin with a working pure CSS dropdown menu based on nested unordered lists. For each li:hover selector in your CSS, add an equivalent li.sfHover selector. Use <PrismCode className="language-css">display:none</PrismCode> to hide submenus. To make this step easy, there are cross-browser CSS files for a variety of menu types included in the Superfish download.
...
<script src="superfish.js"></script>`
			...
			Call <PrismCode className="language-markup">superfish()</PrismCode> on the containing <abbr title="unordered list"><PrismCode className="language-markup">ul</PrismCode></abbr> element.
			<pre><PrismCode className="language-markup">{
`<script>
